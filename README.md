...
This example shows the agent operating on a real user query. It loads a skill generated by the `optimize_anything` process, answers the first question with Unity Catalog functions, and falls back to Genie for the second question because it sits outside the scope of the typed UC tools. The UI is hosted as a Databricks App; the hosting code lives in `app/`.

![At-Bat Assistant Demo](assets/demo.gif)

[Watch the source recording](assets/atbatassistant_demo.mov)
